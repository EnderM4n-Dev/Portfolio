...
(function () {
  ...
  if (!canvas) return;
  // On mobile, skip the background animation on the home page only
  var isMobile = window.matchMedia('(max-width: 768px)').matches;
  var isHome = !!document.getElementById('hero-net');
  if (isMobile && isHome) { canvas.style.display = 'none'; return; }
  ...
  var uNavy  = gl.getUniformLocation(p,'cNavy');
  var uBlue  = gl.getUniformLocation(p,'cBlue');
  var uGold  = gl.getUniformLocation(p,'cGold');
  var uLight = gl.getUniformLocation(p,'cLight');
  function setColors(c) {
    gl.useProgram(p);
    gl.uniform3fv(uNavy,  c[0]);
    gl.uniform3fv(uBlue,  c[1]);
    gl.uniform3fv(uGold,  c[2]);
    gl.uniform3fv(uLight, c[3]);
  }
  window.__setBgColors = setColors;
  setColors(window.__PALETTE_SHADER || [
    [0x21/255,0x35/255,0x55/255],
    [0x4F/255,0x70/255,0x9C/255],
    [0xE5/255,0xD2/255,0x83/255],
    [0xF0/255,0xF0/255,0xF1/255]
  ]);
